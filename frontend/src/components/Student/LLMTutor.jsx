...
import { FaRobot, FaPaperPlane, FaSpinner, FaLightbulb, FaTimes } from 'react-icons/fa';
import api from '../../services/api';

const LLMTutor = ({ performanceData, studentData, isFloating = false }) => {
  ...
  useEffect(() => {
    if (performanceData && performanceData.subjectWiseMarks) {
      const weakSubjects = Object.entries(performanceData.subjectWiseMarks)
        .filter(([_, marks]) => marks < 70)
        .map(([subject]) => subject);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await api.post('/student/llm-tutor', {
      ...
      console.error('Error in LLM tutor:', err);
  ...
  if (isFloating) {
    return (
      <div className={`fixed bottom-6 right-6 z-50 flex flex-col transition-all duration-300 ${isExpanded ? 'w-80 h-96' : 'w-16 h-16'}`}>
        {isExpanded ? (
          <div className="bg-white rounded-xl shadow-xl overflow-hidden flex flex-col w-full h-full border border-indigo-200">
            ...
            <div className="px-2 py-1 bg-gray-100 flex flex-wrap gap-1">
              {suggestions.map((suggestion, index) => (
                <button
                  ...
                  className="text-xs bg-white border border-gray-300 rounded-full px-2 py-1 text-gray-700 hover:bg-indigo-50 hover:border-indigo-300 transition-colors truncate max-w-[120px]"
                >
                  {suggestion}
                </button>
              ))}
            ...
            <div className="p-2 border-t border-gray-200">
              <div className="flex items-center">
                <textarea
                  ...
                  className="flex-1 border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-1 focus:ring-indigo-500 resize-none text-sm"
                  rows="1"
                />
                <button
          ...
          <button 
            onClick={toggleExpand}
            className="w-16 h-16 rounded-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-lg flex items-center justify-center hover:shadow-xl transition-all duration-300 animate-pulse"
          >
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden">
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-4 text-white flex justify-between items-center">
        ...
        <div className="flex space-x-2">
          <button className="text-white hover:bg-white/20 p-1 rounded transition-colors" onClick={() => window.open('/student/ai-insights', '_blank')}>
      ...
      <div className="px-4 py-2 bg-gray-100 flex flex-wrap gap-2">
        {suggestions.map((suggestion, index) => (
          <button
            key={index}
            onClick={() => handleSuggestionClick(suggestion)}
            className="text-xs bg-white border border-gray-300 rounded-full px-3 py-1 text-gray-700 hover:bg-indigo-50 hover:border-indigo-300 transition-colors"
          >
            {suggestion}
          </button>
        ))}
